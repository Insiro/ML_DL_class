...
class KNN:
    ...
    def predict(self, height: int, weight: int):
        distance = "distance"
        self.df[distance] = (self.df[HEIGHT] - height)*(self.df[HEIGHT] -
                                                        height) + (self.df[WEIGHT]-weight)+(self.df[WEIGHT]-weight)
...
df = pd.DataFrame()
df[HEIGHT] = [158, 158, 158, 160, 160, 163, 163, 160,
              163, 165, 165, 165, 168, 168, 168, 170, 170, 170]
df[WEIGHT] = [58, 59,  63, 59, 60,  60, 61,  64,
              64,  61, 62,  65, 62, 63, 66, 63, 64, 68]
df[TARGET] = [M,  M,    M,  M,  M,  M,  M,
